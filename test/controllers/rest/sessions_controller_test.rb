require 'test_helper'
require 'faker'

class Rest::SessionsControllerTest < ActionDispatch::IntegrationTest

  setup do
    @email = Faker::Internet.free_email
    @user_agent = Faker::Internet.user_agent
    @test_image_base64 = ""
    @unauthorized_response_body = "{\"message\":\"No Autorizado\"}"
  end

  test "should return status 401" do

    stub_request(:post, "http://localhost:3001/rest/verify/#{@email}").
  with(
    body: "{\"image\":\"#{@test_image_base64}\"}",
    headers: {
          'Content-Type'=>'application/json',
          'User-Agent'=> @user_agent
    }).
    to_return(status: [401, "No Autorizado"], body: @unauthorized_response_body, headers: {})
    puts response
    post rest_login_url, params: {
                            email: @email,
                            image: @test_image_base64
                          }.to_json,
                          headers: {
                            'Content-Type' => 'application/json',
                            'User-Agent' => @user_agent
                          }

    assert_response 401
    assert_equal @unauthorized_response_body, @response.body
  end
end
